...
class TestDatastoresApi:
    ...
    def test_list_datastore_not_found_with_invalid_tag_category(
        ...
        try:
            ...
            pytest.fail(
                f"付与されていないタグカテゴリを指定してのデータストア取得テストでエラーが発生しました。想定される例外が発生しませんでした。"
            )
            ...
            if "404" in str(e):
                print(
                    "✅ 付与されていないタグカテゴリを指定してのデータストア取得テスト成功: 404エラーが正しく返されました"
                )
            else:
                pytest.fail(
    ...
    def test_list_datastore_not_found_with_invalid_tag(
        ...
        try:
            ...
            # 例外が発生し、以降の行は実行されないことを期待する
            pytest.fail(
                f"付与されていないタグを指定してのデータストア取得テストでエラーが発生しました。想定される例外が発生しませんでした。"
            )
            ...
            if "404" in str(e):
                print(
                    "✅ 付与されていないタグを指定してのデータストア取得テスト成功: 404エラーが正しく返されました"
                )
            else:
                pytest.fail(
